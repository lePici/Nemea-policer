...
ff3_attr_t ff3_negate(ff3_attr_t o)
{
    switch(o) {
...
ff3_attr_t ff3_validate(ff3_type_t type, ff3_oper_t op, char* data, ff3_lvalue_t* info)
{
	ff3_val_t* fl = (ff3_val_t*)data;
 ...
 */
int ff3_oper_eval_V2(char* buf, size_t size, ff3_node_t *node)
{
	const ff3_val_t* const fl = (ff3_val_t*)node->value; // filter node data
	const ff3_rec_t* const rc = (ff3_rec_t*)buf; // record data
	ff3_rec_t hord; //Host byte order converted value
	...
	case FFAT_EQ_AD4:
		if (size == 4) { //realign to 16B
			memset(&hord.ip, 0, sizeof(ff3_ip_t));
			hord.ip.data[3] = rc->ip.data[0];
			rc = &hord;
		} else if (size != sizeof(ff3_ip_t)) {
			return -1;
		}
	*/
	...
	case FFAT_EQ_MAC:
		return !memcmp(&rc->str[0], &fl->str[0], sizeof(ff3_mac_t));
	...
	case FFAT_EQ_AD6:
		...
		return !memcmp(&rc->ip, fl->ip.data, sizeof(ff3_ip_t)); //Exact compare
